...
const commentId = dialogRef.value.data.commentId
const replyData = dialogRef.value.data.replyData as Comment
...
const form = ref<any>({
  ...
  type: 'request',
  comment: replyData.comment as string
})
...
const submitForm = () => {
  ...
  axios.post(`${import.meta.env.VITE_BACKEND}/comment/reply/edit`, form.value, { withCredentials: true })
    .then(response => {
      if (response.data.success) {
        ...
        closeDialog(form.value.comment)
      } else {
    ...
    .catch(error => {
      console.error('[EditReplyError]', error)
      toast.add({
        severity: 'error',
        summary: 'Server',
