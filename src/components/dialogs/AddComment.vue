...
const checkForm = () => {
  errors.value = []
  checkError(errors.value, validDescription(form.value.comment, 75, 500))
...
const submitForm = () => {
  ...
  axios.post(`${import.meta.env.VITE_BACKEND}/comment/add`, form.value, { withCredentials: true })
    .then(response => {
      if (response.data.success) {
        ...
        closeDialog(response.data.comment)
      } else {
    ...
    <panel-form-actions class="md:-mx-6 md:-mb-6">
      ...
      <template #right>
        <button type="submit" class="form-button" :disabled="loading">
          <template v-if="!loading">Pridať komentár</template>
          <template v-else>
            Pridáva sa
